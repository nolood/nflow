...
async fn main() {
    ...
    // Phase 1: Mode-specific initialization
    match mode.as_str() {
        "background" => {
        ...
        _ => {
            // Foreground mode: write PID, log to stderr, handle Ctrl+C
    ...
    // Phase 2: Shared startup (DB, socket server, scheduler — runs for all modes)
    ...
    let event_bus = events::new_event_bus(256);
    ...
    loop {
        ...
        let (actions, progress_actions) = scheduler_loop::scheduler_tick(&conn, Some(&event_bus));
        ...
        if !actions.is_empty() {
            ...
            scheduler_loop::execute_actions(&conn, &actions, Some(&event_bus)).await;
        }
    }
